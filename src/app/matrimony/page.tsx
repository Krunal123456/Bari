...
export default function MatrimonyListing() {
    ...
    useEffect(() => {
        const fetchProfiles = async () => {
            setLoading(true);
            try {
                // Fetch all approved profiles and filter client-side to avoid index issues
                const q = query(collection(db, "matrimony_profiles"), where("status", "==", "approved"));
                ...
                // If user is free (no paid subscription) show limited number/preview only
                if (user) {
                    const sub = await getActiveSubscriptionByUser(user.uid);
                    setIsPaid(!!sub);
                    const vp = await getUserProfile(user.uid);
                    setViewerProfile(vp);

                    // Exclude own profile
                    const filtered = fetched.filter((p) => p.id !== vp?.id);
    ...
    // For free users restrict number of browseable profiles
    const browseLimit = isPaid ? Infinity : 12;
    const visibleProfiles = filteredProfiles.slice(0, browseLimit);
            ...
            <div className="pt-24 pb-12 container mx-auto px-6">
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        {!isPaid && filteredProfiles.length > visibleProfiles.length && (
                            <div className="mt-6 text-center">
                                <p className="text-maroon-700 mb-3">Limited preview for Free members. Upgrade to see more profiles and full details.</p>
                                <Link href="/matrimony/plans" className="px-4 py-2 bg-gold-500 text-maroon-900 rounded">Upgrade to Premium</Link>
                            </div>
                        )}
                    </>
                )}
